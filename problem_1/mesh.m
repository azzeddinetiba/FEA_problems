function [Xtot, Ttot, btot, upper_b, upper_b_1, upper_b_2, upper_b_3, bottom_b] = mesh(A,B,C,D,E)
%-----------------
...
upper_b=find((Xtot(:,2)==(B+1)*10e-2)|((Xtot(:,2)==(D+1)*10e-2)&(Xtot(:,1)~=0)));
upper_b_1=[];
upper_b_2=[];
upper_b_3=[];
for i=1:length(upper_b)
    if Xtot(upper_b(i),2)==10e-2
        ...
        if (Xtot(upper_b(i),1)< (A+C-1)*10e-2 && Xtot(upper_b(i),2)~= (D+1)*10e-2) + (Xtot(upper_b(i),1)== (A+C-1)*10e-2 && Xtot(upper_b(i),2)~= (D+1)*10e-2)
...
c=z;
figure(1);
quadmesh(Ttot,Xtot(:,1),Xtot(:,2),z,1)
view(2)
end 